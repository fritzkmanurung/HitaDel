...
@section('content')
    ...
    <div class="main" style="padding: 10px 15px 0 15px;">
        @if (session('success'))
...
margin-top:10px; margin-bottom: 5vh; background-color: #1775F1; color: white; border-radius: 8px; font-size: 18px; padding: 12px 16px 12px 16px; font-weight: bold;">
        {{ __('Welcome to your home page, ') }} {{'Admin'}}!</h2>
        <div class="card-body" style="text-align:center; margin-bottom:50px;">
            <img src="{{ asset('img/layouts/adminprofile.jpg') }}" alt="Profile Picture"
                class="rounded-circle profile-picture"
                style="width: 150px; height: 150px; border-radius:50%; margin-top:5vh;">
            <h2 class="mt-3">{{ Auth::user()->name }}</h2>
        </div>

        <h3>All Users "Hita Del"</h3>
        @forelse ($users as $user)
            @if ($user->name !== 'Admin')
                <div class="card mb-3" style="margin-top: 10px; padding: 10px 10px 10px 0;">
                    <div class="card-body" style="width: 100%; background: white;
                        border-radius: 10px;
                        box-shadow: 4px 4px 16px rgba(0, 0, 0, .1);  display:flex; align-items:center; padding: 10px;">
                        <a href="{{ route('user-profile-admin', $user->id) }}" class="text-decoration-none" style="display:flex; align-items:center;">
                            <div style="display: flex; align-items:center;">
                                ...
                                @else
                                    ...
                                    <div style="width: 60px; height: 60px; background: #0000; border-radius: 50%; overflow: hidden; display: flex; align-items: center; justify-content: center;">
                                        <i class='bx bx-user' style="font-size: 30px; color: #888;"></i>
                                    </div>
                                @endif
                                <p style="color: black;" class="ml-3">{{ $user->name }}</p>
                            </div>
                        </a>
                    </div>
                </div>
            @endif
            @empty
                <p>No users found with the name "Hita Del"</p>
            @endforelse
        </div>
        <br>
        <hr>
        ...
        style="width:95%; text-align:center; margin-left:20px;
        margin-top:5px; margin-bottom: 5px; background-color: #1775F1; color: white; border-radius: 0; font-size: 18px; padding: 12px 16px 12px 16px; font-weight: bold;">User Post</p>
                <br>
                <hr>
        <div class="container" style="padding: 0 70px 0 10px;">
                    ...
                    <div class="card mb-4" style="margin-top: 5vh;">
                        <div class="card-body">
                            ...
                            <div class="user-info mt-2" style="margin-top: 20px; display:flex;">
                                <a href="{{ route('user-profile-admin', $post->user->id) }}">
                                ...
                                @else
                                    <!-- Gantilah dengan elemen visual atau ikon placeholder yang sesuai -->
                                    <div style="width: 50px; height: 50px; background: #0000; border-radius: 50%; overflow: hidden; display: flex; align-items: center; justify-content: center;">
                                        <i class='bx bx-user' style="font-size: 25px; color: #888;"></i>
                                    </div>
                                @endif
                                </a>
                                <div
                                        ...
                                        <div class="head" style="padding: 5px;">
                                            ...
                                            <div class="menu">
                                                ...
                                                <ul class="menu-link" style="">
                                                    ...
                                                    <li>
                                                        <form method="POST" action="{{ route('admin-delete-post', $post->id) }}" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button style="margin-left:17px; background:white; cursor:pointer;" type="submit" class="btn btn-danger">Delete Post</button>
                                                        </form>
                                                                                                                                    <!-- Tampilkan tombol delete post -->
                                                    ...
                                                    <div class="head" style="">
                                                        <div>
                                                            <li id="comment-{{ $post->id }}-{{ $comment->id }}" class="mb-2">
                                                                <strong style="">
                                                                    <a style="display: flex; align-items:center; color: black" href="{{ route('user-profile-admin', $comment->user->id) }}">
                                                        ...
                                                        <div class="menu">
                                                            ...
                                                            <ul class="menu-link">
                                                                <li>
                                                                    <form method="POST"
                                                                    action="{{ route('admin-delete-comment', $comment->id) }}" class="d-inline">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button style="margin-left:17px; background:white; cursor:pointer;" type="submit" class="btn btn-sm btn-danger">Delete</button>
                                                                </form>
                                                                </li>
                                                            </ul>
                                                        </div>
