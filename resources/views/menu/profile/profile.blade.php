    ...
    <div class="main" style="padding: 0px 15px 0 15px;">
        @if (session('success'))
            <div class="alert alert-success" role="alert"
                style="margin-bottom: 5vh; background-color:rgba(141, 255, 131, 0.605); border-radius:5px; width:100%; height:5vh; border:5px; padding: 10px 10px 10px 10px">
                {{ session('success') }}
            </div>
        @endif
            ...
            <div class="row justify-content-center">
                <div class="card-body" style="text-align:center;">
                    <div class="cover-photo" style="padding: 70px 20px 20px 20px; display:flex; justify-content:center;">
                        @if (Auth::user()->background_picture)
                            <img style="max-height: 550px; border-radius:10px;"
                                src="{{ asset('storage/' . Auth::user()->background_picture) }}" alt="Background Picture"
                                class="img-fluid">
                        @else
                        <i class='bx bx-image' style="font-size: 550px; color: #ccc; max-height: 550px; border-radius:10px;"></i>
                        @endif
                    </div>
                    ...
                    <div class="profile-section text-center mt-4">
                        @if (Auth::user()->profile_picture)
                            <img src="{{ asset('storage/' . Auth::user()->profile_picture) }}" alt="Profile Picture"
                                ...
                                style="border-radius: 50%; width: 200px; height: 200px; position:relative; top: -220px; padding:3px; background-color:white;">
                        @else
                            <i class='bx bxs-user-circle' style="background:#ffffff; font-size: 150px; color: #ccc; border-radius: 50%; width: 200px; height: 200px; position:relative; top: -220px; padding:3px; border: 2px solid white;"></i>
                        @endif
                        <h2 style="position:relative; top: -180px" class="mt-3">{{ Auth::user()->name }}</h2>
                        <a style="position:relative; top: -140px; margin-bottom: 5vh; background-color: #1775F1; color: white; border-radius: 14px; font-size: 14px; padding: 12px 16px 12px 16px; font-weight: bold;"
                            href="{{ route('edit-profile') }}" class="btn btn-success">Edit Profile</a>
                    </div>
                </div>
                <div class="user-details mt-4" style="position:relative; top: -70px;">
                    <p><strong style="margin-right: 12vh;">Birthdate</strong>: {{ Auth::user()->birthdate }}</p>
                    <br>
                    <p><strong style="margin-right: 13vh;">Location</strong>: {{ Auth::user()->location }}</p>
                    <br>
                    <p><strong style="margin-right: 10vh;">Description:</strong>: {{ Auth::user()->description }}</p>
                </div>
                <hr>
                <div class="card-body" style="margin-top:5vh;">
                    <p class="btn btn-primary mb-3"
                        style=" text-align:center;
                        margin-top:20px; margin-bottom: 5vh; background-color: #1775F1; color: white; border-radius: 8px; font-size: 18px; padding: 12px 16px 12px 16px; font-weight: bold;">
                        My
                        Post</p>
                        ...
                        <div class="card mb-4" style="margin-top: 5vh;">
                            <div class="card-body">
                                ...
                                <div class="user-info mt-2" style="margin-top: 20px; display:flex;">
                                    ...
                                    @else
                                        <i class='bx bxs-user-circle' style="font-size: 50px; color: #ccc;"></i>
                                    @endif
                                    </a>
                                    <div
                                                    ...
                                                    <li>
                                                        @can('delete', $post)
                                                            <form method="POST" action="{{ route('delete-post', $post->id) }}"
                                                                ...
                                                                @method('DELETE')
                                                                <button
                                                                    style="margin-left:17px; background:white; cursor:pointer;"
                                    ...
                                    box-shadow: 4px 4px 16px rgba(0, 0, 0, .1); padding: 20px 20px 20px 20px;
                                        "
                                            ...
                                            <form id="comment-form-{{ $post->id }}"
                                                onsubmit="event.preventDefault(); commentPost({{ $post->id }});"
                                            ...
                                            @forelse ($post->comments as $comment)
                                                ...
                                                <div class="head" style="">
                                                    <div>
                                                        <li id="comment-{{ $post->id }}-{{ $comment->id }}"
                                                            class="mb-2">
                                                            <strong style="">
                                                                <a style="display: flex; align-items:center; color: black"
                                                                    href="{{ route('user-profile', $comment->user->id) }}">
                                                            ...
                                                            <p style="width: 100%; padding: 0 0 0 60px;">
                                                                {{ $comment->content }}</p>
                                                    </div>
                                                    <div class="menu">
                                                        <i class='bx bx-dots-horizontal-rounded icon'></i>
